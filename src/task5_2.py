...
if __name__ == "__main__":
    ...
    for i, sentence in enumerate(sentences):
        ...
        grammar_file = os.path.join(rules_nbest_dev,'grammar.{}'.format(i))
        with open(grammar_file, 'r') as f:
            phrasetable = f.readlines()

        osyms_file = os.path.join(task5_2_out_dir,'dev.ja.{}.osyms'.format(i))
        with open(osyms_file, 'w') as f:
            phrasetable_to_osyms(sentence, phrasetable, f)

        fst_txt_file = os.path.join(task5_2_out_dir,'grammar.{}.fst.txt'.format(i))
        with open(fst_txt_file, 'w') as f:
            phrasetable_to_fst(sentence, phrasetable, weight_map, f)

        fst_file = os.path.join(task5_2_out_dir,'grammar.{}.fst'.format(i))
        ...
        subprocess.call(['fstarcsort',
                         ...
                         fst_file,fst_file])
        #
        # dot_file = os.path.join(task5_2_out_dir, 'grammar.{}.dot'.format(i))
        # subprocess.call(['fstdraw',
        #                  '--portrait=true',
        #                  fst_file, dot_file])
        #
        # png_file = os.path.join(task5_2_out_dir, 'grammar.{}.png'.format(i))
